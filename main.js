...
devices = await SerialPort.list().then(function(ports){
  ports.forEach(function(portData){
	...
	if(portData["vendorId"]==="1a86" && portFound==false)
	{
		portFound = "yes";
	...
	if(token!="none")
	{
		try{
			...
			request({ 
				...
				method: 'POST',
				url: 'http://www.planterai.com:4000/channels/mychannel/chaincodes/token_erc721'}, push_cb);
...
function update_readings(){
	request({ 
		...
		method: 'GET',
		url: "http://www.planterai.com:4000/channels/mychannel/chaincodes/token_erc721?fcn=ClientMintedNFTs&args='[]'"}, fetch_cb);
			...
			for (let i in results) {
				...
				new_html += `<tr>
					...
					<td>
						...
						<br />
						Temperature:${uriObj["temperature"]}
						<br />
						Humidity:${uriObj["humidity"]}
					</td>
					<td>
...
ipcMain.on('generatePassword', (event, data) => {
  ...
  // });
    ...
    request({ 
      ...
      method: 'POST',
      url: 'http://www.planterai.com:4000/users/login'}, callback);
...
ipcMain.on('registerUserRegister', (event, data) => {
  const myArr = data.split("+++++");
    ...
    request({ 
      ...
      method: 'POST',
      url: 'http://www.planterai.com:4000/register'}, callback);
      function callback(error, response, body) {
        if (!error && response.statusCode == 200) {
			console.log(body);
            const obj = JSON.parse(body);
